...
async function llmAnswerOpenRouter({ secret, question, history }) {
...
function detectIntent(questionNorm) {
  ...
  if (q.includes("что загад") || q.includes("что зага") || q.includes("какой секрет") || q.includes("назови") || q.includes("подскажи") || q.includes("что это за")) {
    return { type: "alwaysUnknown", key: "secretReveal" };
  }

  if (q.includes("какого цвет") || q.includes("цвет") || q.includes("бел") || q.includes("черн") || q.includes("тёмн") || q.includes("темн") || q.includes("красн") || q.includes("син") || q.includes("зел")) {
...
module.exports = async function handler(req, res) {
  try {
    ...
    const { gameId, question } = parseJsonBody(req);
    ...
    if (guessAttempt) {
      ...
      if (byFeatures !== "не знаю") {
        ...
        answer = await llmAnswerOpenRouter({
          secret: game.secret,
    ...
    const nextLeft = Math.max(0, Number(game.left || 0) - 1);
    game.left = nextLeft;
